...
export function App() {
  ...
  return (
    <div className="grid h-full grid-rows-[auto_1fr_auto] bg-background text-foreground">
      <StatusBar cwd={session.cwd} model={session.model} permissionMode={session.permissionMode} sessionId={session.sessionId} status={status} onReset={reset} />
      <Thread scrollRef={scrollRef} onScroll={handleScroll} messages={messages} toolResults={toolResults} live={live} status={status} lastResult={lastResult} error={error} />
