...
export function DiffView(props: DiffViewProps) {
  ...
  return (
    <div className={cn('max-h-96 overflow-auto bg-muted/40 py-1 font-mono text-xs leading-relaxed scrollbar-thin', flush ? 'border-t border-border' : 'rounded-md border border-border')}>
      {hunks.map((hunk, hi) => (
        <div key={hi} className={cn(hi > 0 && 'border-t border-dashed border-border/70')}>
...
function DiffRow(props: DiffRowProps) {
  ...
  return (
    <div className={cn('flex', line.type === 'add' && 'bg-emerald-500/10', line.type === 'del' && 'bg-red-500/10')}>
      <span
        className={cn('w-6 shrink-0 select-none text-center', line.type === 'add' && 'text-emerald-400', line.type === 'del' && 'text-red-400', line.type === 'context' && 'text-muted-foreground/40')}
      >
        {sign}
      </span>
      <span
